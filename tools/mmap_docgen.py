...
if __name__ == "__main__":
    ...
    # argument parser
    arg_parser = argparse.ArgumentParser(description='Generate documentation'
    ...
    for name, reg in mmap.registers.items():
        doc.append(MarkDownHeader('{} register details'.format(name),
                                  level=2), newline=True)
        ...
        for field in reg.fields:
            if 'description' in field.properties:
                ...
                descr = '---'
            reg_fields.add_line(field.dumps_slice(), field.name,
                                field.permissions, descr)
