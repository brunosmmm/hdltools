...
import math
from hdltools.abshdl.codegen import HDLCodeGenerator, indent
...
class VHDLCodeGenerator(HDLCodeGenerator):
    ...
    def gen_HDLModulePort(self, element, **kwargs):
        ...
        vec = self.dump_element(element.vector, evaluate=evaluate)
    ...
    def gen_HDLModule(self, element, **kwargs):
        ...
        if len(element.parameters) > 0:
            ret_str += "generic("
            ret_str += ";\n".join(
                [self.dump_element(p, last=True) for p in element.parameters]
            )
            ret_str += ");"
